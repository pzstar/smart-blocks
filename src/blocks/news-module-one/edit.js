...
export default function Edit(props) {
    ...
    const postInner = (post, index) => {
        ...
        const titleClass = index == 0 ?
            `sb-large-title ${getFontClass(featuredTypographyFamily, featuredTypographyWeight, featuredTypographyTextTransform, featuredTypographyTextDecoration)}` :
            `sb-big-title ${getFontClass(sideTypographyFamily, sideTypographyWeight, sideTypographyTextTransform, sideTypographyTextDecoration)}`;
        ...
        const image_size = index == 0 ? featured_image_size : side_image_size;
        const excerpt_length = index == 0 ? featuredExcerptLength : sideExcerptLength;
        const image_height = index == 0 ? featuredImageHeight : sideImageHeight;
        const display_category = index == 0 ? featuredPostCategory : sidePostCategory;
        return (
                ...
                <div className="sb-post-thumb">
                    ...
                    {display_category && post.categories && (
                        <ul class="post-categories">
                            {post.categories && index == 0 ? post.categories.map((catId, index) => (catInner(catId, index, false))) : post.categories.map((catId, index) => (catInner(catId, index, true)))}
                        </ul>
                    )}
                </div>
        ...
        <>
            ...
            </style>
            {
                headerTitleTypographyFamily && (headerTitleTypographyFamily != 'Default') && (
                    <GoogleFontLoad family={headerTitleTypographyFamily} weight={headerTitleTypographyWeight.replace("italic", "i")} />
                )
            }
            {
                categoryTypographyFamily && (categoryTypographyFamily != 'Default') && (
                    <GoogleFontLoad family={categoryTypographyFamily} weight={categoryTypographyWeight.replace("italic", "i")} />
                )
            }
            {
                featuredTypographyFamily && (featuredTypographyFamily != 'Default') && (
                    <GoogleFontLoad family={featuredTypographyFamily} weight={featuredTypographyWeight.replace("italic", "i")} />
                )
            }
            {
                sideTypographyFamily && (sideTypographyFamily != 'Default') && (
                    <GoogleFontLoad family={sideTypographyFamily} weight={sideTypographyWeight.replace("italic", "i")} />
                )
            }
            {
                excerptTypographyFamily && (excerptTypographyFamily != 'Default') && (
                    <GoogleFontLoad family={excerptTypographyFamily} weight={excerptTypographyWeight.replace("italic", "i")} />
                )
            }
            {
                metasTypographyFamily && (metasTypographyFamily != 'Default') && (
                    <GoogleFontLoad family={metasTypographyFamily} weight={metasTypographyWeight.replace("italic", "i")} />
                )
            }
            <InspectorControls>
            ...
            <div id={id}>
                <div {...useBlockProps({
                    className: "sb-blocks sb-news-module-one"
                })}>
